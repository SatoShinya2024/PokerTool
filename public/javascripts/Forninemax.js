...
let players_card = [dummy, dummy, dummy, dummy, dummy, dummy, dummy, dummy, dummy, dummy, dummy, dummy, dummy, dummy, dummy, dummy, dummy, dummy];
let array_number = 0;
let card_id;
let clicked = false;
...
const river_line_array = new Array(river_L01, river_L02, river_L03, river_L04, river_L05, river_L06, river_L07, river_L08, river_L09,
                                    ...
                                    river_L30);
/*プレイヤーのアクションの順番の初期値*/
preflop_L01.playerArray = ["UTG", "EP1", "EP2", "LJ", "HJ", "CO", "BTN", "SB"];
...
/*フォールドはしていないが、他のプレイヤーのアクション待ちプレイヤーの配列*/
let last_player = "BB";
/*プリフロアクションの初期値*/
...
//ストラドル込みで、L01のプレイヤー配列の順序入れ替え
function pfL01_player_array(){
    preflop_L01.waitPlayer = ["BB"];
    //プレイヤーの人数に応じて、プレイヤー選択肢配列を決定。
    preflop_L01.playerArray = ["UTG", "EP1", "EP2", "LJ", "HJ", "CO", "BTN", "SB"];
    let stradle = Number(document.getElementById("stradle").value);
    for(let i = 0; i < stradle; i++){
        //player_arrayの一番最後にwaitPlayerの一番前の要素を入れる。
        preflop_L01.playerArray.push(preflop_L01.waitPlayer[0]);
        //waitPlayerの一番前にplayer_arrayの一番前の要素を入れる。
        preflop_L01.waitPlayer.push(preflop_L01.playerArray[0]);
        preflop_L01.playerArray.shift();
        preflop_L01.waitPlayer.shift();
    }
    last_player = preflop_L01.waitPlayer[0];
}    
...
function raise_or_bet(value){
    if(value == "raise" || value == "bet"){
        ...
        document.getElementById("describe_dialog_input").value = 2**(Number(document.getElementById("stradle").value) + 1);
    }else{
        document.getElementById("describe_dialog_input").disabled = true;
...
function action_register(l1, l2, who_select, action_select, how_much){
    ...
    if(action_select == "call" || action_select == "All-in"){
        All_player.forEach(function(value){
            if(value.position == who_select && bet_amount_past >= value.stack + value.paychip){
                action_select = "All-in";
    ...
    //アクションがベットとレイズとオールイン以外の場合、額を表示しない
    if(action_select == "raise" || action_select == "bet" || action_select == "All-in"){
...
let All_player = [UTG, EP1, EP2, LJ, HJ, CO, BTN, SB, BB];
//テーブルの初期状態を記録。
let pot;
let minbet;
let minraise;
...
function initialize(){
    //各プレイヤーのスタックと名前を定義
    //各プレイヤーのスタックを記録
    UTG.stack = document.getElementById("UTG_stack").value;
    EP1.stack = document.getElementById("EP1_stack").value;
    EP2.stack = document.getElementById("EP2_stack").value;
    LJ.stack = document.getElementById("LJ_stack").value;
    HJ.stack = document.getElementById("HJ_stack").value;
    CO.stack = document.getElementById("CO_stack").value;
    BTN.stack = document.getElementById("BTN_stack").value;
    SB.stack = document.getElementById("SB_stack").value;
    BB.stack = document.getElementById("BB_stack").value;
    //各プレイヤーに名前が定義されていればpersonal_nameを更新
    UTG.name = document.getElementById("UTG_person").value;
    EP1.name = document.getElementById("EP1_person").value;
    EP2.name = document.getElementById("EP2_person").value;
    LJ.name = document.getElementById("LJ_person").value;
    HJ.name = document.getElementById("HJ_person").value;
    CO.name = document.getElementById("CO_person").value;
    BTN.name = document.getElementById("BTN_person").value;
    ...
    //ストラドルの支払い。ストラドラーの人数によって分岐
    let number_of_stradoler = document.getElementById("stradle").value;
    if(number_of_stradoler >= 1){
        bet(UTG, minbet * 2);
        bet_amount = minbet * 2;
    }
    if(number_of_stradoler >= 2){
        bet(EP1, minbet * 4);
        bet_amount = minbet * 4;
    }
    if(number_of_stradoler >= 3){
        bet(EP2, minbet * 8);
        bet_amount = minbet * 8;
    }
    if(number_of_stradoler >= 4){
        bet(LJ, minbet * 16);
        bet_amount = minbet * 16;
    }
    if(number_of_stradoler >= 5){
        bet(HJ, minbet * 32);
        bet_amount = minbet * 32;
    }
    if(number_of_stradoler >= 6){
        bet(CO, minbet * 64);
        bet_amount = minbet * 64;
    }
    ...
    if(document.getElementById("ante_who").value == "BigBlind"){
        ...
        pot += document.getElementById("ante_amount").value * 6;
    }
    ...
    //プレイヤーのハンドの設定。
    UTG.card1 = players_card[0];
    UTG.card2 = players_card[1];
    EP1.card1 = players_card[2];
    EP1.card2 = players_card[3];
    EP2.card1 = players_card[4];
    EP2.card2 = players_card[5];
    LJ.card1 = players_card[6];
    LJ.card2 = players_card[7];
    HJ.card1 = players_card[8];
    HJ.card2 = players_card[9];
    CO.card1 = players_card[10];
    CO.card2 = players_card[11];
    BTN.card1 = players_card[12];
    BTN.card2 = players_card[13];
    SB.card1 = players_card[14];
    SB.card2 = players_card[15];
    BB.card1 = players_card[16];
    BB.card2 = players_card[17];
}    
//canvas関連のメソッド
...
function draw_card(ctx, player){
    ...
    switch(player.position){
        case "UTG": 
            card1 = document.getElementById("UTG_card1");
            card2 = document.getElementById("UTG_card2");
            card1_x = 165;
            card1_y = 52;
            card2_x = 192;
            card2_y = 52;
            break;
        case "EP1":
            card1 = document.getElementById("EP1_card1");
            card2 = document.getElementById("EP1_card2");
            card1_x = 315;
            card1_y = 52;
            card2_x = 342;
            card2_y = 52;
            break;
        case "EP2":
            card1 = document.getElementById("EP2_card1");
            card2 = document.getElementById("EP2_card2");
            card1_x = 582;
            card1_y = 52;
            card2_x = 610;
            card2_y = 52;
            break;
        case "LJ": 
            card1 = document.getElementById("LJ_card1");
            card2 = document.getElementById("LJ_card2");
            card1_x = 712;
            card1_y = 180;
            card2_x = 739;
            card2_y = 180;
            break;
        case "HJ":
            card1 = document.getElementById("HJ_card1");
            card2 = document.getElementById("HJ_card2");
            card1_x = 582;
            card1_y = 312;
            card2_x = 609;
            card2_y = 312;
            break;
        case "CO": 
            card1 = document.getElementById("CO_card1");
            card2 = document.getElementById("CO_card2");
            card1_x = 432;
            card1_y = 312;
            card2_x = 459;
            card2_y = 312;
            break;
        case "BTN": 
            card1 = document.getElementById("BTN_card1");
            card2 = document.getElementById("BTN_card2");
            card1_x = 166;
            card1_y = 312;
            card2_x = 193;
            card2_y = 312;
...
document.getElementById("stradle").addEventListener("change", function(){
    AllDlete();
    draw_situation("canvas", previous_line(present_line));
})
...
//勝利プレイヤーの名前を集めた配列を設定
let winner_array = [];
//ショーダウンでの勝利をキャンバスに表示する。代替はline_drawと同じで、sign_drawのところだけ変える
...
function open_text_dialog(){
    ...
    setting_text_l03 += "　" + "ストラドラー：" + document.getElementById("stradle").value + "人";
    ...
    let players_inner = " ";
    let UTG = {position:"UTG", name:"UTG", stack:document.getElementById("UTG_stack").value};
    let EP1 = {position:"EP1", name:"EP1", stack:document.getElementById("EP1_stack").value};
    let EP2 = {position:"EP2", name:"EP2", stack:document.getElementById("EP2_stack").value};
    let LJ = {position:"LJ", name:"LJ", stack:document.getElementById("LJ_stack").value};
    let HJ = {position:"HJ", name:"HJ", stack:document.getElementById("HJ_stack").value};
    let CO = {position:"CO", name:"CO", stack:document.getElementById("CO_stack").value};
    let BTN = {position:"BTN", name:"BTN", stack:document.getElementById("BTN_stack").value};
    ...
    const players_array = [UTG, EP1, EP2, LJ, HJ, CO, BTN, SB, BB];
    initialize();
    for(item of All_player){
        ...
        players_inner += '<div class="col-4 border">' + '<div>' + player_name + '</div>' + '<div>' + stack + document.getElementById("unit").value + '</div>' + '<div>' + card1 + " " + card2 + '</div>' + '</div>';
    }
...
function submit_form(){
    ...
    //フォームをsubmit
    document.myform.action = "./page05_2_9";
