...
router.post('/', async function(req, res, next){
    ...
    const userhands = await prisma.handhistory.findMany({where:{userid:id}});
    const handid = userhands[userhands.length-1].id;
    
    await prisma.historysetting.create({
        data:{
            handid:handid,
    ...
    await prisma.historyboard.create({
        data:{
            handid:handid,
    ...
    for(let i=1; i < 30; i++){
        await prisma.historypreflop.create({
            data:{
                handid:handid,
                street:"preflop",
                num:i,
    ...
    for(let i=1; i < 30; i++){
        await prisma.historyflop.create({
            data:{
                handid:handid,
                street:"flop",
                num:i,
    ...
    for(let i=1; i < 30; i++){
        await prisma.historyturn.create({
            data:{
                handid:handid,
                street:"turn",
                num:i,
    ...
    for(let i=1; i < 30; i++){
        await prisma.historyriver.create({
            data:{
                handid:handid,
                street:"river",
                num:i,
...
//         rh29:req.body.rh29,
//     }
//     res.render('page05_3_6test', data);
// })
